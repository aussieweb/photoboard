 ...
 */
	...
	function photoboard_start_browser_session() {
		if(!isset($_SESSION)) {
			session_start();
		}
	}
	add_action('init', 'photoboard_start_browser_session');


	// Store alert message in browser session
	function photoboard_set_alert_message( $category, $code, $message ) {
		$_SESSION[$category][$code] = $message;
	}


	// Get alert message from browser session
	function photoboard_get_alert_message( $category, $code ) {
		$message = '';
