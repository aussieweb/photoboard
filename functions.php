 ...
 */
	...
	function keel_load_theme_files() {
		...
		if ( isset($_COOKIE['fontsLoaded']) && $_COOKIE['fontsLoaded'] === 'true' ) {
			wp_enqueue_style( 'keel-theme-fonts', '//fonts.googleapis.com/css?family=Open+Sans:400,400i,700', null, null, 'all' );
		}
	}
		...
		?>
			<script>
				...
				<?php if ( !isset($_COOKIE['fontsLoaded']) || $_COOKIE['fontsLoaded'] !== 'true' ) : ?>
					loadCSS('//fonts.googleapis.com/css?family=Open+Sans:400,400i,700');
	...
	function keel_register_menus() {
		register_nav_menus(
			array(
				'primary' => __( 'Primary Menu - Logged In' ),
				'primary_logged_out' => __( 'Primary Menu - Logged Out' ),
					...
					<figure>
						<?php
							if ( $args['avatar_size'] !== 0 ) {
								$has_avatar = keel_validate_avatar( get_comment_author_email() );
								if ( $has_avatar ) {
	...
	require_once( dirname( __FILE__) . '/includes/keel-photoboard.php' ); // Photoboard functionality
