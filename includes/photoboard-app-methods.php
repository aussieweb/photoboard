<?php
	...
	function photoboard_can_user_view( $user_id, $media_id ) {
		...
		foreach ($groups as $group) {
			...
			$group_has_access = is_array( $visibility ) && in_array($group->slug, $visibility) ? $visibility[$group->slug] : false;
	...
	function photoboard_get_post_vids($id) {
		...
		if ($videos) {
			...
			foreach ($videos as $video) {
				...
				// If video is missing atts, skip to the next video
				if ( !isset( $video->guid ) ) return;
				...
				$video_caption = isset( $video->post_excerpt ) ? $video->post_excerpt : '';
	...
	function photoboard_set_post_type_on_save( $post ) {
		...
		if ( empty( $post ) || !isset( $post->ID ) || get_post_type( $post->ID ) !== 'post' || get_post_status( $post->ID ) !== 'publish' || wp_is_post_autosave( $post->ID ) ) return;
	...
	function photoboard_create_zip_on_save( $post ) {
		...
		// If post is not published, bail
		if ( empty( $post ) || !isset( $post->ID ) || get_post_type( $post->ID ) !== 'post' || get_post_status( $post->ID ) !== 'publish' ) return;
	...
	function photoboard_add_photo_visibility_checkbox($form_fields, $post) {
		...
		foreach ($groups as $group) {
			...
			$checked = ( is_array( $img_visibility ) && array_key_exists( $group_id, $img_visibility ) && $img_visibility[$group_id] === 'no' ? '' : 'checked' );
			$html .=
				'<label class="setting">' .
