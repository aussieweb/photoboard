...
<article <?php if ( !is_singular() ) { echo 'class="grid-dynamic" data-right-height-content'; } ?>>
	...
	?>
		...
		<div class="clearfix">
			...
			<?php if ( false ) : // if ( is_single() && get_post_meta( $post->ID ) !== '' ) : ?>
				<?php $track_event = 'onClick="_gaq.push([\'_trackEvent\', \'Album\', \'Download\', \'' . get_the_title($post->ID) . '\']);"'; ?>
				<a class="btn btn-secondary float-right margin-bottom" <?php echo $track_event; ?> href="<?php echo wp_get_attachment_url( get_post_meta( $post->ID, 'photoboard_download_zip', true ) ) . $post->post_name . '.zip'; ?>" download>
					<svg class="icon">
